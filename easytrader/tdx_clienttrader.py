...
from easytrader.clienttrader import IClientTrader
...
class TDXClientTrader(IClientTrader):
    """
    2021.03.11:
    以通达信为核心的交易软件
    """
    ...
    grid_strategy: Union[IGridStrategy,
                         Type[IGridStrategy]] = grid_strategies.TDXXls
    ...
    def connect(self, exe_path=None, **kwargs):
        """
        直接连接登陆后的客户端
        :param exe_path: 客户端路径类似 r'C:\\c:\new_hbzq\\\tdxw.exe', 默认
        :return:
        """
        connect_path = exe_path or self._config.DEFAULT_EXE_PATH
        if connect_path is None:
            raise ValueError(
                "参数 exe_path 未设置，请设置客户端对应的 exe 地址,类似 C:\\客户端安装目录\\tdxw.exe"
            )

        self._app = pywinauto.Application(
            backend="uia").connect(path=connect_path, timeout=10)
    ...
    @property
    def asset(self):
        self._switch_left_menus(["查询", "资金股份"])
        balance, positions = self._get_grid_data(
            self._config.OUTPUT_CONTROL_ID)
        _asset = {
            'balance': balance,
            'positions': positions
        }
    ...
    def today_entrusts(self):
        self._switch_left_menus(["查询", "当日委托"])

        return self._get_grid_data(self._config.OUTPUT_CONTROL_ID, False)

    @property
    def today_trades(self):
        self._switch_left_menus(["查询", "当日成交"])
    ...
    def _set_trade_params(self, security, price, amount):
        ...
        self._type_edit_control_keys(
            self._config.TRADE_SECURITY_CONTROL_ID, code, class_name='AfxWnd42')
    ...
    def _get_grid_data(self, control_id, is_asset=True):
        return self.grid_strategy_instance.get(control_id, is_asset)
    ...
    def _type_edit_control_keys(self, control_id, text, class_name='Edit'):
        if not self._editor_need_type_keys and class_name == 'Edit':
            self._main.child_window(
    ...
    def _switch_left_menus(self, path, sleep=0.2):
        self.close_pop_dialog()
        self._get_left_menus_handle().get_item(path).click_input()
        self.wait(sleep)
        ...
        while True:
            try:
                handle = self._main.child_window(
                    control_id=0xE900, class_name="SysTreeView32"
                )
                if count <= 0:
                    return handle
